// src/components/ui/SolvejetLogo.tsx
import React from 'react';
import { cn } from '@/lib/utils';

interface SolvejetLogoProps {
  className?: string;
  width?: number | string;
...
export const SolvejetLogo: React.FC<SolvejetLogoProps> = ({
  className,
  width = 180,
  height = 60,
  withText = true,
  ...
  return (
    ...
    >
      <title id="solvejet-logo-title">Solvejet Logo</title>
      <g>
        {withText && (
          <>
            {/* S */}
            ...
            />
            {/* L */}
            <path d="m518.7 187.4h50.7v245.9h-50.7z" fill={color.primary} />
            {/* V */}
            <path
        ...
        {/* Dot pattern - these are the dots on the right side */}
        <path
...
// A smaller icon version that only shows the dot pattern
export const SolvejetIcon: React.FC<Omit<SolvejetLogoProps, 'withText'>> = props => {
  return <SolvejetLogo {...props} withText={false} />;
};
